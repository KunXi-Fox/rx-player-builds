...
const { getPictureOnPictureStateRef, getVideoVisibilityRef, getElementResolutionRef, getScreenResolutionRef } = events;
/**
...
class Player extends EventEmitter {
    ...
    _priv_onDecipherabilityUpdate(contentInfos, elts) {
        ...
        const periodsAndTrackTypes = elts.reduce((acc, elt) => {
            var _a, _b, _c;
            const isFound = arrayFind(acc, (x) => x[0].id === elt.period.id &&
        ...
        for (const [period, trackType] of periodsAndTrackTypes) {
            this._priv_triggerEventIfNotStopped("representationListUpdate", { period: { start: period.start,
                    ...
                    id: period.id },
                trackType,
                reason: "decipherability-update" }, contentInfos.currentContentCanceller.signal);
        }
    }
    /**
