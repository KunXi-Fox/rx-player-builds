...
export default class RebufferingController extends EventEmitter {
    ...
    start() {
        ...
        this._playbackObserver.listen((observation) => {
            ...
            const { BUFFER_DISCONTINUITY_THRESHOLD, FREEZING_STALLED_DELAY, UNFREEZING_SEEK_DELAY, UNFREEZING_DELTA_POSITION } = config.getCurrent();
            if (freezing !== null) {
                ...
                if (!position.isAwaitingFuturePosition() &&
                    ...
                    this._playbackObserver.setCurrentTime(this._playbackObserver.getCurrentTime() + UNFREEZING_DELTA_POSITION);
            ...
            if (stalledPosition !== null &&
                ...
                if (skippableDiscontinuity !== null) {
                    ...
                    if (realSeekTime <= this._playbackObserver.getCurrentTime()) {
                        log.info("Init: position to seek already reached, no seeking", this._playbackObserver.getCurrentTime(), realSeekTime);
                    }
                    else {
