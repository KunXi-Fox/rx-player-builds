 ...
 * doing.
 */
export default function PeriodStream({ bufferType, content, garbageCollectors, playbackObserver, representationEstimator, segmentFetcherCreator, segmentBuffersStore, options, wantedBufferAhead, maxVideoBufferSize }, callbacks, parentCancelSignal) {
    ...
    adaptationRef.onUpdate((choice) => {
        ...
        (async () => {
                ...
                if (segmentBufferStatus.type === "initialized") {
                    ...
                    if (SegmentBuffersStore.isNative(bufferType)) {
                        ...
                        if (period.start > periodEnd) {
                            ...
                            await segmentBufferStatus.value.removeBuffer(period.start, periodEnd);
            ...
            const segmentBuffer = createOrReuseSegmentBuffer(segmentBuffersStore, bufferType, adaptation);
    ...
    function createAdaptationStream(adaptation, representations, segmentBuffer, cancelSignal) {
        ...
        AdaptationStream({ content: { manifest,
            ...
            wantedBufferAhead,
            maxVideoBufferSize }, Object.assign(Object.assign({}, callbacks), { error: onAdaptationStreamError }), cancelSignal);
...
function getFirstDeclaredMimeType(adaptation) {
    ...
    if (representations.length === 0) {
        const noRepErr = new MediaError("NO_PLAYABLE_REPRESENTATION", "No Representation in the chosen " +
            adaptation.type + " Adaptation can be played", { tracks: [toTaggedTrack(adaptation)] });
        throw noRepErr;
    }
