 ...
 */
async function getDurationFromManifest(url, transport) {
    var _a;
    if (transport !== "dash" &&
        transport !== "smooth" &&
    ...
    if (transport === "dash" || transport === "smooth") {
        ...
        if (transport === "dash") {
            ...
            const firstDASHStartAttribute = (_a = periodElements[0]) === null || _a === void 0 ? void 0 : _a.getAttribute("start");
            const firstDASHStart = firstDASHStartAttribute !== null ? parseDuration(firstDASHStartAttribute) :
                0;
    ...
    const { responseData } = response;
    const metaplaylist = JSON.parse(responseData);
