...
function parseCueBlock(cueLines, timeOffset) {
    ...
    if ((0, is_non_empty_string_1.default)(cueLines[1]) && cueLines[1].indexOf("-->") !== -1) {
        _a = __read(cueLines[1].split("-->")
            .map(function (s) { return s.trim(); }), 2), startTimeString = _a[0], endTimeString = _a[1];
        payload = cueLines.slice(2, cueLines.length);
    }
    if (!(0, is_non_empty_string_1.default)(startTimeString) ||
        !(0, is_non_empty_string_1.default)(endTimeString)) {
        // Try to see if we find them in the first position
        _b = __read(cueLines[0].split("-->")
            .map(function (s) { return s.trim(); }), 2), startTimeString = _b[0], endTimeString = _b[1];
