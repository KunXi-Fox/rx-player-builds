...
function refreshScheduledEventsList(oldScheduledEvents, manifest) {
    ...
    for (var i = 0; i < periods.length; i++) {
        ...
        streamEvents.forEach(function (_a) {
            ...
            var actualData = { type: data.type,
                value: __assign(__assign({}, data.value), { element: element }) };
            if (end === undefined) {
                var newScheduledEvent = { start: start, id: id, data: actualData,
                    publicEvent: { start: start, data: actualData } };
                scheduledEvents.push(newScheduledEvent);
            }
            else {
                var newScheduledEvent = { start: start, end: end, id: id, data: actualData,
