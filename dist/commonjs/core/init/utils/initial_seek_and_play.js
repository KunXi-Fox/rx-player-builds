...
var browser_detection_1 = require("../../../compat/browser_detection");
...
function performInitialSeekAndPlay(_a, cancelSignal) {
    ...
    var autoPlayResult = new Promise(function (resolveAutoPlay, rejectAutoPlay) {
        ...
        if (!isDirectfile || typeof startTime === "number") {
            var initiallySeekedTime = typeof startTime === "number" ? startTime :
                startTime();
            ...
            playbackObserver.listen(function (obs, stopListening) {
                if (obs.readyState >= 1) {
                    stopListening();
                    var initiallySeekedTime_1 = typeof startTime === "number" ? startTime :
                        startTime();
                    if (initiallySeekedTime_1 !== 0) {
                        if (browser_detection_1.isSafariMobile) {
        ...
        function waitForSeekable() {
            ...
            playbackObserver.listen(function (obs, stopListening) {
                ...
                if ((hasAskedForInitialSeek && !hasStartedSeeking) ||
                    obs.readyState === 0) {
                    return;
                }
                stopListening();
